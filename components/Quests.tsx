...
const Quests = () => {
    ...
    const getCategoryColor = (cat: string) => {
        switch (cat) {
            case 'DAILY': return 'text-rose-400 bg-rose-400/10 border-rose-400/20';
            ...
            {sortedQuests.map((userQuest) => {
                ...
                return (
                    <div
                        ...
                        ${themeMode === 'dark'
                                ? 'bg-[#050000]/80 border-white/5 backdrop-blur-md hover:border-white/10'
                    ...
                    >
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex items-center text-rose-400 font-bold font-mono text-sm bg-rose-500/10 px-2 py-1 rounded border border-rose-500/20">
                        ...
                        <div className="space-y-3 text-center">
                            <div className={`flex justify-between text-xs font-mono ${themeMode === 'dark' ? 'text-zinc-500' : 'text-zinc-600'}`}>
                                <span>PROGRESS</span>
                                <span className={userQuest.completed ? 'text-rose-500' : ''}>{userQuest.progress} / {questDef.target}</span>
                            </div>
                            <div className={`h-2 w-full rounded-full overflow-hidden ${themeMode === 'dark' ? 'bg-zinc-800' : 'bg-zinc-200'}`}>
                                <div
                                    className={`h-full transition-all duration-1000 ${userQuest.completed ? 'bg-rose-500' : 'bg-rose-500'}`}
                                    style={{ width: `${percent}%` }}
                                ></div>
                            </div>
                            ...
                            {userQuest.completed ? (
                                userQuest.claimed ? (
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold uppercase tracking-widest shadow-[0_0_15px_rgba(244,63,94,0.4)] animate-pulse transition-all ${loadingQuestId === userQuest.questId ? 'opacity-60 cursor-not-allowed' : ''}`}
                                    >
