...
const Queue = () => {
    ...
    const getRoleIcon = (role: string) => {
        if (role.toLowerCase().includes('duelist')) return <Zap className="w-3 h-3" />;
        if (role.toLowerCase().includes('sentinel')) return <Shield className="w-3 h-3" />;
                    ...
                    {isAdmin && (
                        <div className="flex space-x-2">
                            ...
                            <button
                                ...
                                disabled={!isInQueue}
                                className={`text-[10px] uppercase transition-colors ${isInQueue
