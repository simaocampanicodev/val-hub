...
const MatchHistory = () => {
  ...
  const getBadgeStyle = (match: MatchRecord) => {
      ...
      return 'bg-rose-600 text-white border-rose-600 shadow-[0_0_10px_rgba(225,29,72,0.4)]';
  };
                  ...
                  <Card key={match.id} className={`border-l-4 transition-all ${getResultColor(match)}`} noPadding>
                      ...
                      >
                          ...
                          <div className="w-1/3 text-left">
                              <h3 className={`font-display font-bold text-lg ${match.winner === 'A' ? 'text-emerald-500' : 'text-zinc-500'}`}>
                                Team {match.captainA}
                              </h3>
                              {match.winner === 'A' && <span className="text-[10px] bg-emerald-500/20 text-emerald-500 px-2 py-0.5 rounded uppercase">Winner</span>}
                          </div>
                          ...
                          <div className="w-1/3 text-right flex flex-col items-end">
                              <h3 className={`font-display font-bold text-lg ${match.winner === 'B' ? 'text-emerald-500' : 'text-zinc-500'}`}>
                                Team {match.captainB}
                              </h3>
                              {match.winner === 'B' && <span className="text-[10px] bg-emerald-500/20 text-emerald-500 px-2 py-0.5 rounded uppercase">Winner</span>}
                          </div>
